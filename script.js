...
// Mobile spring bounce animation on scroll boundary hit
(function initMobileScrollBounce() {
  let startY = 0;
  ...
  window.addEventListener('touchmove', (e) => {
    ...
    const atBottom = (scrollTop + clientHeight >= scrollHeight - 4) && deltaY < 0;
    
    if (atTop || atBottom) {
      const container = document.querySelector('.app-container');
      if (container) {
        const pullAmount = Math.min(Math.abs(deltaY) * 0.18, 28);
        const direction = atTop ? 1 : -1;
        container.style.transition = 'transform 0.08s cubic-bezier(0.16, 1, 0.3, 1)';
        container.style.transform = `translate3d(0, ${pullAmount * direction}px, 0)`;
      }
    }
  }, { passive: true });
  ...
  const resetSpring = () => {
    ...
    const container = document.querySelector('.app-container');
    if (container) {
      container.style.transition = 'transform 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.35)';
      container.style.transform = 'translate3d(0, 0px, 0)';
    }
  };

  window.addEventListener('touchend', resetSpring, { passive: true });
  window.addEventListener('touchcancel', resetSpring, { passive: true });
})();
