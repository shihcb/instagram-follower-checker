// -------------------------------------------------------------
// Global Console Log & Error Redirector for Remote UI Debugging
// -------------------------------------------------------------
(function() {
  function printToOverlay(type, args) {
    ...
    line.style.borderBottom = '1px dashed rgba(0, 255, 0, 0.15)';
  ...
  console.log = function() {
    ...
    printToOverlay('log', arguments);
  };

  const oldError = console.error;
  console.error = function() {
    oldError.apply(console, arguments);
    printToOverlay('error', arguments);
  };

  const oldWarn = console.warn;
  console.warn = function() {
    oldWarn.apply(console, arguments);
    printToOverlay('warn', arguments);
  };

  window.onerror = function(message, source, lineno, colno, error) {
    let errText = `Unhandled: ${message} at ${source}:${lineno}:${colno}`;
    if (error && error.stack) {
      errText += `\nStack: ${error.stack}`;
    }
    printToOverlay('error', [errText]);
    return false;
  };
})();
